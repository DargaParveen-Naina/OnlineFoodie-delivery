    ...
    featuredItems.forEach(food => {
        ...
        foodCard.innerHTML = `
            ...
            <div class="food-info">
                ...
                <span class="price">₹${(food.price*USD_TO_INR).toFixed(0)}</span>
...
function updateCartUI() {
    ...
    cart.forEach(item => {
        ...
        cartItem.innerHTML = `
            <div class="item-info">
                <h4>${item.name}</h4>
                <p class="item-price">₹${(item.price*USD_TO_INR).toFixed(0)} each</p>
            </div>
            ...
            <p class="item-total">₹${(itemTotal*USD_TO_INR).toFixed(0)}</p>
    ...
    // cartTotal.textContent = `₹${(total * USD_TO_INR).toFixed(0)}`;
    cartTotal.innerHTML = `₹${(total * USD_TO_INR).toFixed(0)}`;
...
function checkout() {
    ...
    const order = {
        items: [...cart],
        total: Number(cartTotal.textContent.replace("₹", "")),
...
`;
    ...
    order.items.forEach(item => {
        const itemTotal = item.price * item.quantity * USD_TO_INR;
        billContent += `${item.name}
        Price: ₹${(item.price*USD_TO_INR).toFixed(0)} x ${item.quantity} = ₹${itemTotal.toFixed(0)}
-------------------------------------
`;
    });
...
// document.addEventListener("DOMContentLoaded", function () {
//     const totalWrapper = document.getElementById("cartTotal")?.parentElement;
//     if (totalWrapper) {
//         totalWrapper.innerHTML = `Total: <span id="cartTotal">₹0</span>`;
//     }
// });
